...
if [ -z "$infakeystorePassword" ] ; then
   echo "$(date) - $0 - Could not retrieve keystore password"
   exit 1
fi

infasetup.sh UpdateGatewayNode -HttpsPort 6443 -KeystoreFile ${infaKeystore} -KeystorePass ${infakeystorePassword}
